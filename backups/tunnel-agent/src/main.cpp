...
void run_agent(const protogate::agent::AgentConfig& config) {
    ...
    while (running && reconnect.should_reconnect()) {
        try {
            ...
            // Create HTTP client (uses HTTPS with Azure Container Apps)
            HttpClient http_client(io_context, config.server.host, config.server.port, true);  // use_tls=true for HTTPS
            ...
            http_client.connect(config.tunnel.id, config.tunnel.token);
            
            // Create HTTP/2 session
            HTTP2Session http2_session(io_context, http_client, config.tunnel.id, config.tunnel.token);
