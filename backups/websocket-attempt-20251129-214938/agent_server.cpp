    ...
    if (!tcp_ports_str.empty()) {
        ...
        while (std::getline(port_stream, port_str, ',')) {
            ...
            if (!port_str.empty()) {
                ...
                tcp_tunnel.target_host = tunnel->target_host;
                tcp_tunnel.target_port = tunnel->target_port;
...
void AgentServer::AgentHandshake::send_websocket_upgrade_response() {
    auto self = shared_from_this();
    
    // Calculate WebSocket accept key (SHA-1 hash of key + magic string, base64 encoded)
    // For simplicity, using fixed accept value matching our fixed key
    std::string ws_accept = "s3pPLMBiTxaQ9kYGzzhZRbK+xOo="; // Accept for "dGhlIHNhbXBsZSBub25jZQ=="
    ...
    boost::asio::async_write(socket_, boost::asio::buffer(response_str),
        [this, self](const boost::system::error_code& ec, std::size_t /*bytes_transferred*/) {
            ...
            observability::Logger::instance().info("WebSocket upgrade successful", {
                {"agent_ip", agent_ip_}
            });
            // Connection remains open for AgentConnection to take over
        });
}
