...
HTTPServer::HTTPServer(
    ...
    : io_pool_(io_pool),
      ...
      router_(router),
      tunnels_handler_(tunnels_handler),
      port_(port),
      running_(false),
      use_tls_(use_tls),
...
void HTTPServer::handle_plain_http(std::shared_ptr<boost::asio::ip::tcp::socket> socket) {
    ...
    boost::asio::async_read_until(*socket, *buffer, "\r\n\r\n",
        [this, socket, buffer](const boost::system::error_code& ec, std::size_t bytes_transferred) {
            ...
            // Check if this is an agent connection request (/v1/agent/connect)
            if (path == "/v1/agent/connect" && method == "POST" && tunnels_handler_) {
                observability::Logger::instance().info("Agent connection request detected", {
                    {"path", path},
                    {"client_ip", client_ip}
                });
                ...
                api_request.body = "";  // No body needed for agent connect
                api_request.remote_ip = client_ip;
                
                // Call agent connection handler
                api::HttpResponse api_response;
                tunnels_handler_->handle_agent_connect(api_request, api_response, socket);
                
                // Check if upgrade succeeded (HTTP 101)
                if (api_response.status_code == 101) {
                    observability::Logger::instance().info("Agent connection upgraded", {
                        {"client_ip", client_ip}
                    });
                    ...
                    boost::asio::async_write(*socket, boost::asio::buffer(response),
                        [socket, client_ip](const boost::system::error_code& ec, std::size_t) {
                            if (ec) {
                                observability::Logger::instance().warning("Error writing agent upgrade response", {
                                    {"error", ec.message()}
                                });
                                socket->close();
                                return;
                            ...
                            observability::Logger::instance().info("Agent upgrade response sent, socket kept open", {
                                {"client_ip", client_ip}
                            });
                            
                            // Socket remains open for agent communication
                            // TODO: Create HTTPAgentConnection wrapper and register with agent_registry
                        });
                    return;
                } else {
                    // Authentication failed - send error response and close
                    std::string response = api::Router::serialize_response(api_response);
                    boost::asio::async_write(*socket, boost::asio::buffer(response),
