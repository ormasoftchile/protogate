...
void TunnelsHandler::register_routes(Router& router) {
    ...
    // Note: /v1/agent/connect requires special handling in HTTPServer
    // to keep socket open instead of closing after response
}
...
std::string TunnelsHandler::generate_token(const std::string& tunnel_id) {
    ...
    token_stream << "tnl_" << std::hex << std::setfill('0');
...
void TunnelsHandler::handle_agent_connect(const HttpRequest& request, HttpResponse& response,
                                          std::shared_ptr<boost::asio::ip::tcp::socket> socket) {
    // Extract Authorization header
    auto auth_it = request.headers.find("authorization");
    if (auth_it == request.headers.end()) {
        response.set_error(401, "Missing Authorization header");
        return;
    }
    
    std::string authorization = auth_it->second;
    
    // Validate token
    auto result = token_validator_->validate(authorization);
    if (!result.valid) {
        observability::Logger::instance().warning("Agent connection failed - invalid token", {
            {"client_ip", request.remote_ip},
            {"error", result.error_message}
        });
        response.set_error(401, result.error_message);
        return;
    }
    ...
    if (!tunnel) {
        observability::Logger::instance().warning("Agent connection failed - tunnel not found", {
            {"client_ip", request.remote_ip},
            {"tunnel_id", tunnel_id}
        });
        response.set_error(404, "Tunnel not found");
    ...
    if (agent_registry_->is_connected(tunnel_id)) {
        ...
        response.set_error(409, "Agent already connected for this tunnel");
        return;
    }
    
    observability::Logger::instance().info("Agent HTTP connection authenticated", {
        {"tunnel_id", tunnel_id},
        {"client_ip", request.remote_ip},
    ...
    // Send HTTP 101 Switching Protocols response to upgrade connection to WebSocket
    response.status_code = 101;
    response.headers["Upgrade"] = "websocket";
    response.headers["Connection"] = "Upgrade";
    response.headers["Sec-WebSocket-Accept"] = "s3pPLMBiTxaQ9kYGzzhZRbK+xOo=";
    
    // Note: Socket is NOT closed after this response
    // HTTPServer will handle keeping socket open and creating AgentConnection
}
